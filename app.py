...
def cosine_similarity(x, y):
    ...
    cosine_sim = np.dot(x,y)/max(la.norm(x)*la.norm(y),1e-3)
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type=50):
    ...
    for word in words:
        ...
        # If the word is in the embeddings, add its embeddings to the total and increment the counter
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        #TODO: Check if category is in embeddings, if not update category embeddings            
        
        # Compute cosine similarity between input sentence and categories
        ...
        for index in range(len(categories)):
            ...
            if model_name:
                ...
                category_embedding = get_sentence_transformer_embeddings(category)
    ...
    #sorted_cosine_sim = sorted(cosine_sim.values(),reverse=True)

    # Sort cosine similarity
    # Alternative workign with dictionary
    #sorted_cosine_sim = dict(sorted(cosine_sim.items(), key=lambda item: item[1], reverse=True))
    # Sort cosine similarity
    sorted_cosine_sim = [[categories.index(item[0]), item[1]] for item in sorted(cosine_sim.items(), key=lambda item: item[1], reverse=True)]

    print(f'Sorted cosine sim = {sorted_cosine_sim}')
...
def plot_piechart_helper(sorted_cosine_scores_items):
    sorted_cosine_scores = np.array(
        [
            sorted_cosine_scores_items[index][1]
            #value
            #for index,value in sorted_cosine_scores_items.items()
            for index in range(len(sorted_cosine_scores_items))
        ]
    )
    categories = st.session_state.categories.split(" ")
    categories_sorted = [
        categories[sorted_cosine_scores_items[index][0]]
        #category
        #for category,score in sorted_cosine_scores_items.items()
        for index in range(len(sorted_cosine_scores_items))
    ]
...
model_type = st.sidebar.selectbox("Choose the model", ("50d",), index=0)
...
if st.session_state.text_search:
    ...
    print(f'Sorted Glove = {sorted_cosine_sim_glove}')
    print(f'Sorted Transform = {sorted_cosine_sim_transformer}')
